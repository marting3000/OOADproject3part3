...
public class Runner extends Adventurer {
	//Private variables only able to be used by the runner class
	private int x;
	private int y;
	private int z;
	private String name;
	...
	//Unique move/fight/loot function to the sneaker
	public ArrayList<Creature> moveFight(ArrayList<Creature> creatures) {
		...
		do {
			...
			if (xyz == 1 && !(availableMoves.get(0).isEmpty())) {
				...
				y = availableMoves.get(2).get(Utilities.generateRandomNumber(0, availableMoves.get(2).size() - 1));
				validMove = true;
			}
		} while(!validMove);
		...
		availableMoves = findAvailableMoves(x, y, z);
		xyz = 0;
		validMove = false;
		monsterFlag = false;
		adventurerFightNum = 0;
		creatureFightNum = 0;
